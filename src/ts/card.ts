...
export function displaycard(task: Task): string {
  ...
  if (task.dueDate) {
    ...
    if (task.status !== "completed") {
      ...
      } else if (state === "soon") dateColorClass = "text-amber-500";
    }
...
function dueDateState(dueDate: string): "normal" | "soon" | "overdue" {
...
export function getTime(createdAt:number): string {
